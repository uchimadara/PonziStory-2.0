...
if ($('.pie')[0]) {
    $('.pie').each(function(){
        ...
        var c = $(this);
        drawPie(c.attr('title'), c.attr('data-url'), c.attr('data-div'));
    });
}

function drawPie(title, uri, div) {
    ...
    $.ajax({
        ...
        success: function (data) {
            var vals = consumeJSONPieData(data);
            $.plot($('#' + div), vals, {
                series: {
                    pie: {
                        show: true,
...
function drawPieChart(title, uri, div) {
    ...
    var cdata = new google.visualization.DataTable(data);

    // Set chart options
    var options = {'title': title,
        width: 300,
        height: 350,
        chartArea: {top: "60", width: "300", height: "350"},
        legend: {position: 'left', textStyle: {color: "#191919", fontSize: 12}},
        backgroundColor: { fill: "#ffffff" },
        titleTextStyle: {color: "#191919", fontSize: 17}
    };
...
function consumeJSONPieData(pieData) {
    var i = 0;
    var vals = new Array();
    for (var key in pieData) {
        vals[i++] = { label: key, data: parseInt(pieData[key]) };
    }
    return vals;
}
