...
<form action="<?= base_url() ?>adverts/assign_credits" class="frm_ajax" method="post" name="acForm">
    ...
    <div id="textAdList" class="pageable sortable getList" data-url="<?= base_url() ?>member/getList/user_text_ads/?user_id=<?= $userData->id ?>">
       <div class="loading"></div>
    </div>
    <? if ($userData->text_ad_credits > 0) { ?>
...
<? if ($ajax) { ?>
    <script>
        $.get('<?= base_url() ?>member/getList/user_text_ads/?user_id=<?= $userData->id ?>', function (data) {
