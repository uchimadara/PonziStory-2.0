...
class webtasks extends My_Controller
{
	...
	public function WebTasksForApproval()
	{
		...
		if ($this->form_validation->run()) {
			...
			if ($approve === 'approve') {
				...
				$statusForEmail = 'Approved';

				$this->sendEmail($user_name_task, $statusForEmail);
			...
			else
			{
				...
				$statusRejectedForEmail = 'Rejected';

				$this->sendEmail($user_name_task, $statusRejectedForEmail);
	...
	public function sendEmail($username, $status)
	{
		...
		$messageforemail = 'You Task Have Been '.$status;

		$this->load->library('email');

		$this->email->from('admin@cryptoadz.com','Cryptoadz Admin');

		$this->email->to($emaill);

		$this->email->subject('Notification Of Task Approval');

		$this->email->message($messageforemail);
