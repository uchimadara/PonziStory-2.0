...
//        $this->addJavascript(asset('bootstrap/js/jquery.flot.pie.min.js'));
    ...
    public function ChangeAdPackPrice()
    {
        ...
        {
            $getname = $this->input->post('changeprice');

            echo $getname;
        }

        echo $this->loadPartialView('changepriceview');
...
//        $this->data->memberTickets = $this->Support->getSummary(NULL, false, $status);
    ...
    public function payment_update($id, $approve) {
        ...
        if ($payment->approved || $payment->rejected) {
            ...
            if ($approve == '1') {
                ...
                $this->User->update($payment->payer_user_id, array('account_level' => $payment->code, 'account_expires' => NULL));
                ...
                $payment->rejected = $data['rejected'] = now();
